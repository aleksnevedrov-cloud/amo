...
flowchart LR
  ...
  E -- нет текста --> O[OCR: Yandex Vision, серверы в РФ; запасной — Tesseract на сервере]
...
- **OCR.** Основной — Yandex Vision; без ключа или по настройке — Tesseract (tesseract.js, WASM, один воркер на
  процесс, только картинки). Оба отдают строки с координатами, поэтому восстановление таблиц (`layoutPage`) общее.
- **152-ФЗ.** Файл целиком уходит только в Yandex Vision (РФ) или остаётся на сервере (Tesseract). В Claude — текст после чистки; фото двери/проёма
...
- **Чертежи DWG/DXF** (`packages/docs/src/dwg.ts`): без растеризации и OCR — DWG конвертируется в DXF внешним
  `dwg2dxf` (LibreDWG, путь `DWG2DXF_BIN`), из DXF берутся надписи TEXT/MTEXT, атрибуты блоков и размеры с
  координатами, дальше — общий `layoutPage` и разбор как у любого документа. Без конвертера — понятная ошибка,
  агент просит PDF.
- **Аналитика** (`dialog_outcomes`, `AnalyticsRepo`): при первом контакте AI со сделкой запоминается стартовый этап;
  задача воркера `refresh-outcomes` (раз в 6 ч, сделки до 60 дней) перечитывает этапы через amo API и считает
  «ушла вперёд по воронке» по порядку этапов (`sort`), выигрыш (142) и проигрыш (143). Сводка — по журналу:
  диалоги, ответы, передачи с причинами, стоимость и средняя стоимость диалога, по дням; расход по месяцам —
  учёт для биллинга (тарифов по решению заказчика пока нет).
- **Версии настроек**: `settings_audit` уже хранил `before/after` с автором; добавлены история с перечнем
  изменённых разделов, снимок и откат (откат — новая версия).
- **Мультиаккаунт**: изоляция по `account_id` из проверенного токена; удаление всех данных аккаунта по запросу
  каскадом (`POST /widget/v1/account/purge`, админ + подтверждение).
- **Качество (раздел 14)**: 52 eval-диалога (в т. ч. 7 на инъекции и выманивание цены), нагрузочный прогон
  `evals/perf.ts` (100 одновременных диалогов), проверка секретов в `pnpm check`/CI, `docs/security-review.md`.
